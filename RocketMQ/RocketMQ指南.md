...
- 若现未知状态 （本地事务状态为UNKNOW），则会触发工行系统的本地事务状态`回查操作`。会查操作将会回查结果，即COMMIT_MESSAGE或ROLLBACK_MESSAGE Report给TC，TC将结果上报给TM，TM会再向TC发送最终确认指令Global Commit 或Global Rollback
...
>
> 以上方案并不是一个典型得XA模式。因为XA模式中得分支事务是异步得，而事务消息方案中得消息预提交与预扣款操作间是同步的。

#### 3、基础

**分布式事务**

对于分布式事务，通俗的说就是，一次操作由若干分支操作组成，这些分支操作分属于不同应用，分布在不同服务器上。分布式事务需要保证这些分支操作要么全部成功，要么全部失败。分布式事务与普通事务一样，为了保证操作结果的一致性。

**事务消息**

RocketMQ提供了类型X/Open XA的分布式事务功能，通过事务消息能达到分布式事务的最终一致。XA是一种分布式解决方案，一种分布式事务处理模式。

**半事务消息**

暂不能投递的消息，发送方已经成功的将消息发送到了Broker，但是Broker未收到最终确认指令，此时该消息被标记成 `暂不能投递状态`，即不能被消费者看到。处于该种状态下的消息即 `半事务消息`
...
> 注意，消息回查不是重新执行回调操作，回调操作时进行预扣款操作，而消息回查则是查看预扣款操作执行的结果。
...
XA（unix transaction）是一种分布式事务解决方案，一种分布式事务处理模式，是基于XA协议的。XA协议由Tuxedo(Transaction for Unix has been Extended for Distributed Operation)` 分布式操作扩展之后的Unix事务系统 ` 首先提出的，并交给 X/Open 组织。作为资源管理器与事务管理器的接口标准。
...
> RocketMQ中消息事务的Producer以及Broker 均是RM
...
5、TC在接收到各个RM的Report后将汇总结果上报给TM，根据汇总结果TM会向TC发出确认指令。
