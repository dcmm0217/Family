...
接口性能优化对于后端开发而言，肯定是再熟悉不过了，因为它是一个跟开发语言无关的公共问题。
...
![image-20211201061313364](https://gitee.com/huangwei0123/image/raw/master/img/image-20211201061313364.png)

**通过这几列可以判断索引使用情况，执行计划包含列的含义如下图所示：**

![image-20211201061402592](https://gitee.com/huangwei0123/image/raw/master/img/image-20211201061402592.png)

这里只是简单提及一下方法，如何详细去使用，还需要深入explain的学习。

**说实话，sql语句没有走索引，排除没有建索引之外，最大的可能性是索引失效了。**

下面说说索引失效的常见原因：

![image-20211201061552530](https://gitee.com/huangwei0123/image/raw/master/img/image-20211201061552530.png)

如果不是上面的这些原因，则需要再进一步排查一下其他原因。

#### 1.3 选错索引

此外，你有没有遇到过这样的情况：明明是同一条SQL，只有入参不同而已，有的时候走的索引A，有的时候却走的索引B

没错，有时候mysql会选错索引

必要的时候可以使用`force index`来强制查询sql走某个索引

至于为什么mysql会选错索引，再这里就不多做介绍了。
...
![image-20211201061930141](https://gitee.com/huangwei0123/image/raw/master/img/image-20211201061930141.png)

可以对照这些技巧去排查自己的sql语句，看是否存在优化的空间。
...
![image-20211201062324910](https://gitee.com/huangwei0123/image/raw/master/img/image-20211201062324910.png)
...
![image-20211201062450708](https://gitee.com/huangwei0123/image/raw/master/img/image-20211201062450708.png)
...
![image-20211201062820667](https://gitee.com/huangwei0123/image/raw/master/img/image-20211201062820667.png)
...
![image-20211201063540204](https://gitee.com/huangwei0123/image/raw/master/img/image-20211201063540204.png)
...
![image-20211201064009408](https://gitee.com/huangwei0123/image/raw/master/img/image-20211201064009408.png)
...
![image-20211201064102579](https://gitee.com/huangwei0123/image/raw/master/img/image-20211201064102579.png)
...
没错，使用@Transactional注解这种**声明式事务**的方式提供事务功能，确实能少写很多代码，提升开发效率。

但也容易造成大事务，引发其他的问题。

下面用一张图看看大事务引发的问题。

![image-20211201064332966](https://gitee.com/huangwei0123/image/raw/master/img/image-20211201064332966.png)
...
我们都知道**文件上传操作是非常耗时**的，如果将整个方法加锁，那么需要等到整个方法执行完之后才能释放锁。显然，**这会导致该方法的性能很差，变得得不偿失**。
...
这种存储结构决定了，想一次性查出这个分类树，并非是一件非常容易的事情。这就需要使用程序递归查询了，如所以，如果每次都直接从数据库中查询分类树的数据，是一个非常耗时的操作。

**这时我们可以使用缓存，大部分情况，接口都直接从缓存中获取数据。操作redis可以使用成熟的框架，比如：jedis和redisson等。果分类多的话，这个递归是比较耗时的。**
...
**此外，我们还需要有个job每隔一段时间，从数据库中查询菜单数据，更新到redis当中，这样以后每次都能直接从redis中获取菜单的数据，而无需访问数据库了。**(其实更好的可以直接缓存失效好了，查到以后再去设置到缓存内)。

![image-20211201070159965](https://gitee.com/huangwei0123/image/raw/master/img/image-20211201070159965.png)
...
![image-20211201070339219](https://gitee.com/huangwei0123/image/raw/master/img/image-20211201070339219.png)
...
![image-20211201070506279](https://gitee.com/huangwei0123/image/raw/master/img/image-20211201070506279.png)
...
![image-20211201071204396](https://gitee.com/huangwei0123/image/raw/master/img/image-20211201071204396.png)
...
![image-20211201071215437](https://gitee.com/huangwei0123/image/raw/master/img/image-20211201071215437.png)
...
![image-20211201071242116](https://gitee.com/huangwei0123/image/raw/master/img/image-20211201071242116.png)

通过skywalking定位性能问题：

![image-20211201071252656](https://gitee.com/huangwei0123/image/raw/master/img/image-20211201071252656.png)
