...
![image-20220308203617255](https://mygiteepic.oss-cn-shenzhen.aliyuncs.com/img/image-20220308203617255.png)
...
Java虚拟机的指令由一个字节长度的、代表着某种特定操作含义的`操作码（opcode）`以及跟随其后的零至多个代表此操作所需参数的`操作数（openrand）`所构成。虚拟机中许多指令并不包含操作数，只有一个操作码。 

![image-20220308204137010](https://mygiteepic.oss-cn-shenzhen.aliyuncs.com/img/image-20220308204137010.png)

#### 2.2 包装类对象的缓存问题

![image-20220308210124078](https://mygiteepic.oss-cn-shenzhen.aliyuncs.com/img/image-20220308210124078.png)
...
class文件的结构并不是一成不变的，随着Java虚拟机的不断发展，总是会不可避免地会对class文件结构做出一些调整，但是其基本结构和框架是非常稳定的。
...
![image-20220308203935931](https://mygiteepic.oss-cn-shenzhen.aliyuncs.com/img/image-20220308203935931.png)
...
![image-20220308212812399](https://mygiteepic.oss-cn-shenzhen.aliyuncs.com/img/image-20220308212812399.png)
...
- 直接引用：直接引用可以是直接`指向目标的指针、相对偏移量或是一个能间接定位到目标的句柄。直接引用是与虚拟机实现的内存布局相关的`，同一个符号引用在不同虚拟机实例上翻译出来的直接引用一般不会相同。如果有了直接引用，那么说明引用的目标必定已经存在于内存之中了。

常量池可以理解成Class文件中的资源仓库。主要存放的是两大类常量：`字面量(Literal)`和`符号引用(Symbolic References)`。
...
![image-20220308213037860](https://mygiteepic.oss-cn-shenzhen.aliyuncs.com/img/image-20220308213037860.png)
