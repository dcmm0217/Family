...
同理，我们实现Callable接口，也需要实现call()方法，但是这个时候我们还需要有返回值，这个Callable接口的应用场景一般就在于批处理业务，比如转账的时候，需要给一会返回结果的状态码回来，代表本次操作成功 还是 失败。
...
这里需要用到的是**FutureTask**类，他实现了Runnable接口，并且还需要传递一个实现**Callable**接口的类作为构造函数
...
![image-20211109151505523](https://mygiteepic.oss-cn-shenzhen.aliyuncs.com/img/image-20211109151505523.png)
...
- 控制运行线程的数量，处理过程中，将任务放到队列中，然后线程创建后，启动这些任务，如果线程数量超过了最大数量的线程排队等候，等其他线程执行完毕，再从队列中取出任务来执行。
- 线程复用、控制最大并发线程、管理线程
- 线程池中的任务是放入阻塞队列中的

#### 线程池的好处

**多核处理的好处：**省略线程上下文切换带来的开销
...
- 提高线程的可管理性。线程时稀缺资源，如果无限创建，不仅会消耗系统资源，还会降低系统的稳定性，使用线程池可以进行统一的分配，调优和监控

#### 架构说明

Java中线程池是通过Executor框架实现的，该框架中用到了Executor，Executors（代表工具类），ExecutorService，ThreadPoolExecutor这几个类。

![image-20211109152948245](https://mygiteepic.oss-cn-shenzhen.aliyuncs.com/img/image-20211109152948245.png)

![image-20211109153001940](https://mygiteepic.oss-cn-shenzhen.aliyuncs.com/img/image-20211109153001940.png)
...
![image-20211109164337235](https://mygiteepic.oss-cn-shenzhen.aliyuncs.com/img/image-20211109164337235.png)
...
![image-20211109164441890](https://mygiteepic.oss-cn-shenzhen.aliyuncs.com/img/image-20211109164441890.png)

#### 线程池的重要参数

![image-20211109165023913](https://mygiteepic.oss-cn-shenzhen.aliyuncs.com/img/image-20211109165023913.png)

线程池在创建的时候，一共有7大参数

- corePoolSize：核心线程数，线程池中的常驻核心线程数
  - 在创建线程池后，当有请求任务来之后，就会安排池中的线程去执行请求任务，近似理解为今日当值线程
  - 当线程池中的线程数目达到corePoolSize后，就会把到达的线程放入缓存队列中
...
![image-20211109183203838](https://mygiteepic.oss-cn-shenzhen.aliyuncs.com/img/image-20211109183203838.png)
...
![image-20211109183434898](https://mygiteepic.oss-cn-shenzhen.aliyuncs.com/img/image-20211109183434898.png)
...
​	 2.3 如果这时候队列满了，并且正在运行的线程数量还小于maximumPoolSize，那么还是创建非核心线程like运行这个任务

​	 2.4 如果队列满了并且正在运行的线程数量大于或等于maximumPoolSize，那么线程池会启动饱和拒绝策略来执行

3、当一个线程完成任务时，它会从队列中取下一个任务来执行

4、当一个线程无事可做>操作一定的时间(keepAliveTime)时，线程池会判断：
...
- 线程资源必须通过线程池提供，不允许在应用中显示的创建线程
  - 使用线程池的好处是**减少在创建和销毁线程上所消耗的时间以及系统资源的开销**，**解决资源不足的问题**，如果不使用线程池，有可能造成系统创建大量同类线程而导致消耗完**内存或者“过度切换”的问题**
...
ExecutorService executorService = new ThreadPoolExecutor(
    ...
    new ThreadPoolExecutor.AbortPolicy());//抛异常拒绝策略
```
...
在生产环境中，如何配置 corepoolSize 和 maxiumPoolSize

这个是根据具体业务来配置的，分为CPU密集型和IO密集型

- CPU密集型

CPU密集型的意思是该任务需要大量的运算，而没有阻塞，CPU一直在全速前进
