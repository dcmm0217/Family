...
队列就可以相成一个数组，从一头进入，一头出去，**排队买饭**

#### 阻塞队列

BlockingQueue   阻塞队列，排队拥堵，首先它是一个队列，而一个阻塞队列在数据结构中所起的作用大致如下图所示：

![image-20211103142333515](https://gitee.com/huangwei0123/image/raw/master/img/image-20211103142333515.png)

线程1往阻塞队列中添加元素，而线程2从阻塞队列中移除元素

- 当阻塞队列是空时，从队列中获取元素的操作将会被阻塞
  - 当蛋糕店的柜子里面是空的时候，无法从柜子里获取蛋糕

- 当阻塞队列是满的时候，从队列中添加数据的操作将会被阻塞
  - 当蛋糕店的柜子里面是满的时候，无法继续向柜子里添加蛋糕了
...
```
synchronizned :   this.wait() 阻塞    this.nofiy() 唤醒  this.nofiyAll() 唤醒全部
ReetrantLock  :  Condition condition = lock.newCondition()  condition.await() 阻塞   condition.signal() 唤醒
Semaphore  :   Semaphore semaphore = new Semaphore(3);  semaphore.acquire() 阻塞  semaphore.release() 释放资源
```

#### 架构：

![image-20211103144119685](https://gitee.com/huangwei0123/image/raw/master/img/image-20211103144119685.png)
...
- SynchronousQueue：不存储元素的阻塞队列，也即单个元素的队列（定制化）
  - 生成一个，消费一个，不存储元素，不消费生产
...
![image-20211103145103426](https://gitee.com/huangwei0123/image/raw/master/img/image-20211103145103426.png)
